...
public class ContainerPreemptionManager {
  ...
  private final Map<ContainerId, Container> containersToKill =
      new HashMap<>();
  ...
  private List<Container> pickOpportunisticContainersToReclaimResources(
    ...
    if (!hasSufficientResources(resourcesToFreeUp)) {
      ...
      LOG.warn("There are no sufficient resources to start guaranteed [{}]"
              ...
              + "being killed to make room. utilization {}", containerToStartId,
          this.utilizationTracker.getCurrentUtilization());
      List<ContainerId> toKill = new ArrayList<>();
      extraOpportContainersToKill.stream()
          .forEach(container -> toKill.add(container.getContainerId()));
      LOG.warn("Containers to kill {}", StringUtils.join(",", toKill));
