...
public class ContainerScheduler extends AbstractService implements
  ...
  public void serviceInit(Configuration conf) throws Exception {
    ...
    this.containerQueueManager.initConfigs(conf);
  }
  ...
  public ContainerScheduler(Context context, AsyncDispatcher dispatcher,
    ...
    this.utilizationTracker =
        ContainerSchedulerUtil.initializeResourceTracker(context, this);
    this.containerQueueManager =
        new ContainerQueueManager(oppContainersQueuePolicy, qLength,
            utilizationTracker, metrics, context);
    this.runningContainersManager = new RunningContainersManager(utilizationTracker, metrics);
    ...
    this.containerPreemptionManager =
          new ContainerPreemptionManager(this.containerQueueManager,
              this.runningContainersManager, context,
              this.utilizationTracker);
  }
  ...
  public void handle(ContainerSchedulerEvent event) {
    ...
    case SHED_QUEUED_CONTAINERS:
      this.containerQueueManager.shedQueuedOpportunisticContainers();
      break;
    case RECOVERY_COMPLETED:
      this.runningContainersManager.startPendingContainers(this.containerQueueManager,
          this.containerQueueManager.getMaxOppQueueLength() <= 0);
      break;
    default:
  ...
  private void onUpdateContainer(UpdateContainerSchedulerEvent updateEvent) {
    ...
    if (updateEvent.isExecTypeUpdate()) {
      ...
      if (updateEvent.isIncrease()) {
        ...
        if (this.containerQueueManager.removeOpportunisticContainerFromQueue(containerId) != null) {
          this.containerQueueManager.addGuaranteedContainerToQueue(updateEvent.getContainer());
          //Kill/pause opportunistic containers if any to make room for
          // promotion request
          this.containerPreemptionManager.reclaimContainerResources(updateEvent.getContainer());
        }
      } else {
        // Demotion of queued container. Should not happen too often
        // since you should not find too many queued guaranteed
        // containers
        if (this.containerQueueManager.removeGuaranteedContainerFromQueue(containerId) != null) {
          this.containerQueueManager.addOpportunisticContainerToQueue(updateEvent.getContainer());
        }
      }
      this.runningContainersManager.startPendingContainers(this.containerQueueManager,
          this.containerQueueManager.getMaxOppQueueLength() <= 0);
  ...
  public void recoverActiveContainer(Container container,
    ...
    if (rcs.getStatus() == RecoveredContainerStatus.QUEUED
      ...
      LOG.info("Recovering running containers {} utilization {}",
          container.getContainerId(),
          utilizationTracker.getCurrentUtilization());
    }
    if (rcs.getStatus() != RecoveredContainerStatus.COMPLETED
        && rcs.getCapability() != null) {
      metrics.launchedContainer();
      metrics.allocateContainer(rcs.getCapability());
      metrics.launchedContainer(execType);
    }
  }
  ...
  public void setUsePauseEventForPreemption(
    ...
    this.containerPreemptionManager.setUsePauseEventForPreemption(usePauseEventForPreemption);
  ...
  private void onResourcesReclaimed(Container container) {
    ...
    Container queued =
        this.containerQueueManager.removeOpportunisticContainerFromQueue(container.getContainerId());
    if (queued == null) {
      this.containerQueueManager.removeGuaranteedContainerFromQueue(container.getContainerId());
    }
    ...
    if (resourceReleased) {
      ...
      this.runningContainersManager.logRunningContainers();
    }
  }
  ...
  protected void scheduleContainer(Container container) {
    ...
    if (isGuaranteedContainer) {
      ...
      this.runningContainersManager.startPendingContainers(
          this.containerQueueManager, this.containerQueueManager.getForceStartGuaranteedContainers());
      // if the guaranteed container is queued, we need to preempt opportunistic
      // containers for make room for it
      if (this.containerQueueManager.isGuaranteedContainerQueued(container.getContainerId())) {
      ...
      this.runningContainersManager.startPendingContainers(this.containerQueueManager,false);
      boolean containerQueued = this.containerQueueManager.enqueueContainer(container);
  ...
  public void updateQueuingLimit(ContainerQueuingLimit limit) {
    this.containerQueueManager.setMaxOpportunisticQueueingLimit(limit.getMaxQueueLength());
