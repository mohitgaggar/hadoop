...
/**
 * ContainerQueueManager class is responsible for maintaining opportunistic
 * and guaranteed container queues.
 */
public class ContainerQueueManager {
  ...
  private Boolean opportunisticQueueLengthLimitDisabled = false;
  ...
  public void initConfigs(Configuration config) {
    this.opportunisticQueueLengthLimitDisabled = config.getBoolean(
        YarnppConfiguration.YPP_OPPORTUNISTIC_QUEUELENGTH_LIMIT_DISABLED,
        YarnppConfiguration.DEFAULT_YPP_OPPORTUNISTIC_QUEUELENGTH_LIMIT_DISABLED);
  }

  /**
   * Get queued guaranteed containers
   *
   * @return LinkedHashMap containing the id to container mapping of queued
  ...
  public boolean enqueueContainer(Container container) {
    ...
    if (isGuaranteedContainer) {
      ...
      case BY_QUEUE_LEN:
      default:
        if (isOpportunisticQueueLengthLimitDisabled()) {
          isQueued = true;
        } else if (maxOppQueueLength <= 0) {
          isQueued = false;
        } else {
